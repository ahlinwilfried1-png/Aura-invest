  ...
  return (
    <div className="animate-fadeIn max-w-2xl mx-auto space-y-5 pb-24 text-slate-900">
      ...
      <div className="relative flex items-center justify-between py-2 px-1">
        ...
        <h1 className="text-lg sm:text-xl font-bold text-slate-900 text-center tracking-tight flex-1">
          Preuves de retrait
        </h1>
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[#FEDB87] via-[#FEBA4F] to-[#FA9E2C] p-3.5 sm:p-4 shadow-2xs border border-amber-300/40 flex items-center justify-between min-h-[90px]">
        <div className="max-w-[70%] space-y-1 z-10">
          <h2 className="text-slate-950 font-extrabold text-xs sm:text-sm leading-snug tracking-wide font-sans uppercase">
            TÉLÉCHARGEZ VOTRE BON DE RETRAIT POUR DES RÉCOMPENSES EN ESPÈCES
          </h2>
          <p className="text-[10px] sm:text-xs text-amber-950/90 font-medium leading-tight">
            Partagez vos captures de retrait validées et gagnez des bonus exclusifs.
          </p>
        </div>

        {/* Money Bag & Gold Coins Illustration */}
        <div className="absolute -right-2 -bottom-2 w-24 sm:w-28 h-24 sm:h-28 pointer-events-none select-none flex items-center justify-center">
      ...
      <div className="space-y-2.5">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-[11px] font-bold bg-slate-900 hover:bg-slate-800 text-white px-3 py-1.5 rounded-lg transition-all shadow-2xs flex items-center space-x-1 cursor-pointer"
          >
        ...
        {/* Upload Form Box */}
        {showUploadForm && (
          <form onSubmit={handleFormSubmit} className="bg-white rounded-xl border border-slate-200 p-3.5 space-y-3 shadow-2xs animate-fadeIn">
            <div className="flex items-center space-x-2 text-[11px] font-semibold text-emerald-700 bg-emerald-50 p-2 rounded-lg border border-emerald-200">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: 2000"
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-900 font-bold text-xs outline-none focus:border-slate-900"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Ex: Merci beaucoup AURA INVEST très bonne application"
                className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-slate-900 text-xs outline-none focus:border-slate-900 resize-none"
                required
              />
            </div>
            ...
            <div>
              ...
              <div className="border border-dashed border-slate-200 hover:border-slate-400 rounded-lg p-2.5 text-center cursor-pointer bg-slate-50/50 relative">
                <input
                  type="file"
                  accept="image/*"
                ...
                {imageInput ? (
                  <div className="flex items-center justify-center space-x-2">
                    <img src={imageInput} alt="Aperçu" className="h-12 w-12 object-cover rounded-md border border-slate-300" />
                    <span className="text-[11px] font-bold text-emerald-600">Image chargée !</span>
                  </div>
                ) : (
                  <div className="flex flex-col items-center space-y-0.5">
                    <Upload className="w-4 h-4 text-slate-400" />
                    <span className="text-[11px] font-medium text-slate-600">Cliquez pour importer la preuve</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-red-500 hover:bg-red-600 text-white font-bold text-xs rounded-lg transition-all shadow-2xs flex items-center justify-center space-x-1.5 cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="space-y-2.5 pt-1">
        ...
        {approvedProofs.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-6 text-center space-y-1">
            ...
            <p className="text-[11px] font-medium text-slate-500">Aucune preuve publiée pour le moment.</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {approvedProofs.map((proof) => {
              const formattedPhone = proof.userPhone;
              ...
              return (
                <div
                  key={proof.id}
                  className="bg-white rounded-xl border border-slate-200/90 p-3 sm:p-3.5 shadow-2xs transition-all relative overflow-hidden"
                >
                  <div className="flex items-start justify-between gap-2.5">
                    ...
                    <div className="space-y-1 flex-1 min-w-0 pr-1">
                      <div className="flex items-baseline justify-between flex-wrap gap-x-2">
                        ...
                        <span className="font-black text-red-600 text-xs sm:text-sm tracking-tight font-sans">
                          {formattedAmount}
                        </span>
                      </div>
                      ...
                      {proof.message && (
                        <p className="text-slate-700 font-medium text-[11px] sm:text-xs leading-normal pt-0.5 font-sans">
                          {proof.message}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="shrink-0">
                      {proof.imageUrl ? (
                        <div className="w-14 sm:w-18 h-18 sm:h-24 rounded-lg border border-slate-800 overflow-hidden bg-slate-950 shadow-2xs relative group cursor-pointer">
                          <img
                            src={proof.imageUrl}
                        ...
                        <div className="w-14 sm:w-18 h-18 sm:h-24 rounded-lg border border-slate-200 bg-slate-50 flex flex-col items-center justify-center p-1 text-center text-slate-400">
                          <CheckCircle2 className="w-4 h-4 text-emerald-500 mb-0.5" />
                          <span className="text-[9px] font-bold text-slate-500 leading-tight">Confirmé</span>
                        </div>
                      )}
                    </div>
                  </div>
