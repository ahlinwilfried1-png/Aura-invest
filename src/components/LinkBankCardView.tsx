...
export const LinkBankCardView: React.FC<LinkBankCardViewProps> = ({
  ...
  return (
    <div className="animate-fadeIn max-w-xl mx-auto space-y-6 pb-24 text-slate-900 font-sans">
      ...
      {/* 2. Frameless Intro Banner directly on page background */}
      ...
      {/* 3. Frameless Form (No Box Outline, No Outer Cards or Borders) */}
      <form onSubmit={handleSubmit} className="space-y-5 pt-2">
            ...
            {ALLOWED_COUNTRIES.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-3.5 py-3 rounded-2xl text-xs font-bold transition-all cursor-pointer ${
                    isSel
                      ? 'bg-amber-500 text-slate-950 font-black shadow-xs scale-[1.02]'
                      : 'bg-slate-200/60 text-slate-700 hover:bg-slate-200/90'
                  }`}
                >
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            className="w-full bg-slate-100/80 focus:bg-white border-0 focus:ring-2 focus:ring-amber-500 rounded-2xl px-4 py-3.5 text-sm font-bold text-slate-900 outline-none transition-all placeholder:text-slate-400 placeholder:font-normal"
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center bg-slate-100/80 focus-within:bg-white focus-within:ring-2 focus-within:ring-amber-500 rounded-2xl px-4 py-3.5 transition-all">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="**** (Ex: 1234)"
            className="w-full bg-slate-100/80 focus:bg-white border-0 focus:ring-2 focus:ring-amber-500 rounded-2xl px-4 py-3.5 text-sm font-mono font-bold text-slate-900 outline-none transition-all placeholder:text-slate-400 placeholder:font-sans placeholder:font-normal"
          />
        ...
        {isSaved && (
          <div className="p-3.5 bg-emerald-100/80 text-emerald-900 rounded-2xl flex items-center space-x-2.5 text-xs font-bold animate-fadeIn">
            <CheckCircle2 className="w-4 h-4 text-emerald-700 shrink-0" />
            <span>Votre carte bancaire / compte de paiement est lié et enregistré.</span>
          </div>
        )}

        {/* Large Frameless Action Button */}
        <button
          type="submit"
          className="w-full py-4 bg-gradient-to-r from-amber-500 via-amber-500 to-amber-600 hover:brightness-105 active:scale-[0.99] text-slate-950 font-black text-sm uppercase tracking-wider rounded-2xl shadow-sm cursor-pointer transition-all flex items-center justify-center space-x-2"
        >
