...
export const ChatMessenger: React.FC<ChatMessengerProps> = ({
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    // Create Support Ticket in App state
    createSupportTicket("Message Chat Support", messageToSend, imageToSend);

    // Simulate instant auto-reply from Customer Support if auto-replying is triggered
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] min-h-[580px] max-w-4xl mx-auto bg-white rounded-3xl overflow-hidden relative font-sans">
      
      {/* 1. CHAT HEADER / SUPPORT BAR */}
      <div className="bg-white px-4 py-3 sm:px-6 border-b border-slate-200/80 flex items-center justify-between z-10">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h3 className="text-sm sm:text-base font-black text-slate-900 tracking-tight">
                Service Client <span className="text-amber-700">AURA INVEST</span>
              </h3>
      ...
      {/* 2. CONVERSATION CANVAS AREA (SEAMLESS TEXT LAYOUT ON CLEAN BACKGROUND) */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 bg-white">
        ...
        <div className="max-w-2xl mx-auto space-y-6 text-slate-900 font-sans text-sm sm:text-base leading-relaxed py-2">
          ...
          <div className="space-y-3 font-medium text-slate-900">
            <h2 className="text-base sm:text-lg font-black text-slate-900 flex items-center space-x-2">
              <span>🚗✨</span> <span>AURA INVEST — Avancez vers la réalisation de vos rêves !</span>
            </h2>
          ...
          <p className="font-extrabold text-amber-800 text-sm sm:text-base flex items-center space-x-2 pt-2">
            <span className="text-lg">💪</span>
            <span>Rejoignez AURA INVEST et ouvrez la voie à un avenir brillant !</span>
          </p>

          {/* Bullet Points Section */}
          <div className="space-y-3 pt-4 text-xs sm:text-sm">
            <p className="font-bold text-slate-900 flex items-center space-x-2">
              <span>💡</span> <span>Pourquoi choisir AURA INVEST ?</span>
            </p>
        ...
        {userTickets.map((tkt) => (
          <div key={tkt.id} className="max-w-2xl mx-auto space-y-4 my-6">
            ...
            {/* Admin or Simulated Support Reply */}
            {tkt.reply ? (
              <div className="flex items-start space-x-3 max-w-xl animate-fadeIn py-2">
                ...
                <div className="space-y-1 text-xs sm:text-sm text-slate-900 font-medium leading-relaxed">
                  ...
                  <div className="text-[9px] text-slate-400 font-mono font-medium pt-1">
              ...
              <div className="flex items-start space-x-3 max-w-xl animate-fadeIn py-2">
                ...
                <div className="space-y-1 text-xs text-slate-800">
                  <div className="flex items-center space-x-1 text-[10px] font-bold text-emerald-700">
                    ...
                    <span>Réception confirmée par le Support</span>
                  </div>
                  <p className="text-slate-700 font-medium leading-relaxed">
                    Bonjour <strong className="text-slate-900">{currentUser.name}</strong> ! Votre message a bien été transmis à nos conseillers. Un agent traite votre demande.
                  </p>
        ...
        {/* Typing indicator when autoReplying */}
        {autoReplying && (
          <div className="flex items-center space-x-2 max-w-xs animate-fadeIn">
            <div className="w-8 h-8 rounded-xl bg-amber-500 text-slate-950 flex items-center justify-center flex-shrink-0 font-black text-xs shadow-sm">
              AI
            </div>
            <div className="bg-white rounded-2xl px-4 py-2.5 border border-teal-100 shadow-xs flex items-center space-x-1.5">
              ...
              <span className="text-[11px] text-slate-500 font-medium ml-1">Conseiller en train d'écrire...</span>
            </div>
          </div>
        )}
      ...
      {selectedImage && (
        <div className="sticky bottom-[60px] bg-white border-t border-teal-100 px-4 py-2 flex items-center justify-between animate-fadeIn z-30">
          <div className="flex items-center space-x-3">
      ...
      {/* 4. MODERN INPUT BAR (STICKY AT BOTTOM OF SCREEN) */}
      <form onSubmit={handleSendMessage} className="sticky bottom-0 bg-white/95 backdrop-blur-md px-3 sm:px-4 py-3 border-t border-slate-200/90 flex items-center space-x-2 sm:space-x-3 z-30 shadow-md">
        ...
        <input 
          type="text" 
          placeholder="Send your questions... / Posez votre question..."
          value={inputText}
